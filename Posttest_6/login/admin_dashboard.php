...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($image_size > 2 * 1024 * 1024) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
            // Simpan data buku dan nama file gambar ke dalam database
            $sql = "INSERT INTO buku (isbn, title, synopsis, image, language, genre, pages_count, book_type, publish_date, author, biography)
                    VALUES ('$isbn', '$title', '$synopsis', '$new_image_name', '$language', '$genre', '$page_count', '$book_type', '$publish_date', '$author', '$biography')";

            if ($conn->query($sql) === TRUE) {
                echo "Buku berhasil ditambahkan pada $upload_time!";
            } else {
